...
def main():
 ...
 for condition in ['intact','blank_vision','retina_disconnected','all_edges_disconnected']:
  ...
  record={'condition':condition,'input_sha256':hashlib.sha256(light.tobytes()).hexdigest(),'action':{k:a[k] for k in ['turn','forward','attack','interact']},'readouts':a['readouts'],'neural_ms':500,'wall_seconds':round(wall,3)}
  if condition=='all_edges_disconnected':assert a['turn']==a['forward']==0 and not a['attack'] and not a['interact']
 ...
 for tick in range(210):
  ...
  active=abs(a['turn'])>1e-9 or abs(a['forward'])>1e-9 or a['attack'] or a['interact'];actions+=int(active);spikes+=int(c.sum())
  if tick%35==0:
   event={'tick':tick+1,'action':{k:a[k] for k in ['turn','forward','attack','interact']},'game':game.observation()};events.append(event);print(event,flush=True)
 assert actions>0
