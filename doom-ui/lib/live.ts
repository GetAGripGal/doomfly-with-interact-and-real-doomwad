...
export type Live = {
    schema: 1; status: string; run_id: string; sequence: number; generated_at_ms: number; condition: string; decoder: string; frame: string;
    ...
    action: { turn: number; forward: number; attack: boolean; interact: boolean }; readouts: Readout[]; total_spikes: number; window_spikes: number; window_ms: number; total_action_ticks: number;
...
export function parseLive(input: unknown): Live {
    ...
    if (!d.action || !finite(d.action.turn) || !finite(d.action.forward) || typeof d.action.attack !== 'boolean' || typeof d.action.interact !== 'boolean' || !Array.isArray(d.readouts) || !d.readouts.every(r => typeof r.id === 'string' && typeof r.type === 'string' && finite(r.spikes) && finite(r.rate_hz))) throw new Error('Invalid neural outputs');
